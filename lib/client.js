...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    if (typeof window !== "undefined" && !window.__DSCT_BANNER_PRINTED__) {
      ...
      console.log(
        "%c dsh-chat-timeline %c v0.1.4 %c ⭐ https://github.com/dwb-ajay/dsh-chat-timeline ",
    ...
    /**
     * 【中文】从成功的 /rewind 命令结果解析目标 seq（与 dsh-rewind 一致：
     * outcome 文案含 "seq <n>"）。
     * (EN) Parse the rewind target seq from a successful /rewind command
     * outcome (same as dsh-rewind: outcome text contains "seq <n>").
     */
    ...
    function isRewindPreviewCommand(command) {
      return typeof command.args === "string" && command.args.includes("preview");
    }

    /**
     * 【中文】计算 dsh-rewind 已从对话视图隐藏的 anchor seq 集合。
     * 逻辑对齐 dsh-rewind-plugin 的 hiddenSeqsOf：成功 rewind 命令的
     * [targetSeq, markerSeq] 闭区间内的节点都应从时间线消失。
     *
     * (EN) Seqs that dsh-rewind has withdrawn from the chat view. Mirrors
     * dsh-rewind-plugin's hiddenSeqsOf: everything in the inclusive
     * [targetSeq, markerSeq] span of a successful rewind is gone.
     */
    function hiddenSeqsOfChat(chat) {
      const hidden = /* @__PURE__ */ new Set();
      if (chat === void 0 || chat === null || typeof chat !== "object") return hidden;
      ...
      for (const key of order) {
        ...
        if (typeof command.seq === "number") hidden.add(command.seq);
        const target = rewindTargetOfOutcome(command.outcome.text);
    ...
    function TimelineRail({ useProjection, sessionId, sessionsService, t }) {
      ...
      if (Array.isArray(projected?.messages) && projected.messages.length > 0) {
        // Prefer projection, but always strip seqs rewind has withdrawn — the
        // host projection also drops surface-replace shadows; this client pass
        // covers stale projection caches and command-row hide ranges.
