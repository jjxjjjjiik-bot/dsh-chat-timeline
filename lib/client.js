/**
 * dsh-chat-timeline — web client half.
 *
 * A 1:1 port of the DeepSeek official web app's ScrollNav (conversation
 * scroll-navigation rail), extracted from chat.deepseek.com's shipped
 * main.js / main.css:
 *
 * - collapsed: a fixed 34×300px vertical rail at right:16px, vertically
 *   centered, with a blurred translucent backdrop; every user message shows
 *   as one 8×2px grey indicator line
 * - hovering the rail expands a panel (bg layer + shadow + border) whose
 ...
 * - the item nearest the reading position is highlighted (brand-blue text)
 * - clicking an item jumps the chat to that message (loads older history on
 *   demand) and flashes the target row
 * - auto-hidden when the session has fewer than 2 user messages
 *
 * Data sources, fastest first: host messageIndex projection -> loaded chat
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    //#region dsh-chat-timeline styles (1:1 port of DeepSeek web ScrollNav)
    const css =
    ...
    //#region dsh-chat-timeline logic
    ...
    /** Normalize one entry to { seq, time, text, key?, id? }. */
    ...
    /** Resolve the chat node's data-chat-anchor-key (direct key or reconstructed). */
    ...
    //#region dsh-chat-timeline components
    function TimelineRail({ useProjection, sessionId, sessionsService, t }) {
      ...
      // Background full-history load: follow the runtime's authoritative
      ...
      // Track the reading position (active item).
      react.useEffect(() => {
    ...
    //#region dsh-chat-timeline plugin body
