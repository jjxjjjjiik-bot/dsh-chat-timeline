...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css =
      ...
      /* 重点节点标记：星标按钮、金色标记线、只看标记过滤 */
      ".dsct_star{display:none;-webkit-appearance:none;appearance:none;background:none;border:none;cursor:pointer;color:rgba(0,0,0,.3);font-size:13px;line-height:20px;margin-left:6px;padding:0 2px;transition:color .15s ease,transform .12s ease;flex-shrink:0}" +
      ".dsct_wrap.dsct_show .dsct_star{display:inline}" +
      ".dsct_star:hover{color:rgba(0,0,0,.65);transform:scale(1.15)}" +
      ".dsct_star.dsct_staron{color:#f5a623}" +
      "body[data-ds-dark-theme] .dsct_star,[data-theme='dark'] .dsct_star,.dark .dsct_star{color:rgba(255,255,255,.25)}" +
      "body[data-ds-dark-theme] .dsct_star:hover,[data-theme='dark'] .dsct_star:hover,.dark .dsct_star:hover{color:rgba(255,255,255,.65)}" +
      "body[data-ds-dark-theme] .dsct_star.dsct_staron,[data-theme='dark'] .dsct_star.dsct_staron,.dark .dsct_star.dsct_staron{color:#f5a623}" +
      ".dsct_item.dsct_marked .dsct_line{background-color:#f5a623;transform-origin:50%;transform:scale(1.5)}" +
      ".dsct_item.dsct_marked.dsct_active .dsct_line{background-color:#f5a623}" +
      ".dsct_filter{display:none;height:34px;min-height:34px;padding:2px 10px 0 12px;justify-content:flex-end;align-items:center;border-bottom:1px solid rgba(0,0,0,.06);box-sizing:border-box}" +
      ".dsct_wrap.dsct_show .dsct_filter{display:flex}" +
      "body[data-ds-dark-theme] .dsct_filter,[data-theme='dark'] .dsct_filter,.dark .dsct_filter{border-bottom-color:rgba(255,255,255,.08)}" +
      ".dsct_filter_btn{cursor:pointer;background:none;border:none;font:inherit;font-size:12px;line-height:18px;color:rgba(0,0,0,.5);padding:2px 8px;border-radius:999px;transition:all .15s ease}" +
      ".dsct_filter_btn:hover{color:rgba(0,0,0,.8);background:rgba(0,0,0,.05)}" +
      ".dsct_filter_btn.dsct_on{color:#c47d0a;background:rgba(245,166,35,.14)}" +
      "body[data-ds-dark-theme] .dsct_filter_btn,[data-theme='dark'] .dsct_filter_btn,.dark .dsct_filter_btn{color:rgba(255,255,255,.5)}" +
      "body[data-ds-dark-theme] .dsct_filter_btn:hover,[data-theme='dark'] .dsct_filter_btn:hover,.dark .dsct_filter_btn:hover{color:rgba(255,255,255,.8);background:rgba(255,255,255,.08)}" +
      ".dsct_empty{display:none;height:40px;align-items:center;justify-content:center;font-size:12px;color:rgba(0,0,0,.45);padding:0 12px}" +
      ".dsct_wrap.dsct_show .dsct_empty{display:flex}" +
      "body[data-ds-dark-theme] .dsct_empty,[data-theme='dark'] .dsct_empty,.dark .dsct_empty{color:rgba(255,255,255,.45)}" +
      "@media (prefers-reduced-motion:reduce){.dsct_nav,.dsct_wrap,.dsct_title,.dsct_line,.dsct_star{transition:none}}" +
    ...
    //#region key-point bookmark feature（重点节点标记）
    const MARKS_PREFIX = "dsh-chat-timeline:marks:";
    function marksStorageKey(sessionId) { return MARKS_PREFIX + (sessionId ?? ""); }
    function readMarks(sessionId) {
      if (typeof localStorage === "undefined" || sessionId == null) return [];
      try {
        const raw = localStorage.getItem(marksStorageKey(sessionId));
    ...
    function writeMarks(sessionId, marks) {
      try { if (sessionId == null) return; localStorage.setItem(marksStorageKey(sessionId), JSON.stringify(marks)); } catch {}
    }
    function markKeyOf(m) {
      const key = anchorKeyOf(m);
      if (typeof key === "string" && key !== "") return "k:" + key;
      if (typeof m.id === "string" && m.id !== "") return "i:" + m.id;
      return "s:" + m.seq;
    }
    //#endregion
    ...
    function TimelineRail({ useProjection, sessionId, sessionsService, t }) {
      ...
      }, [activeIndex, messages.length, isNarrow, marksOnly]);
